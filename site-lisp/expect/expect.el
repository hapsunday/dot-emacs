...
(defvar expect-always-check-info nil)
(defvar expect-cancel nil)		; Dynamic variable
(defvar expect-main-cancel 'no)		; Dynamic variable

;;; Utility macros.

;;; process message point settinels timer always-queries queries
(defun expect-make-info (process message point)
  (list process message point nil nil nil))
...
(defmacro expect-info-always-queries (info)
  `(nth 5 ,info))
(defmacro expect-info-set-always-queries (info queries)
  `(setcar (nthcdr 5 ,info) ,queries))


(defmacro expect-info-queries (info)
  `(nthcdr 6 ,info))
(defmacro expect-info-set-queries (info queries)
  `(setcdr (nthcdr 5 ,info) ,queries))
...
completion."
  (let ((point (make-symbol "point"))
    ...
    `(save-excursion
       (let ((,point (point))
	     ,process-spec expect-process expect-current-info expect-always-check-info
	     expect-main-cancel)
...
(defmacro expect-while (regexp &rest forms)
  "Execute FORMS while REGEXP  has arrived in the buffer."
  `(expect-push-always ,regexp #'(lambda () ,@forms)))
...
(defun expect-wait ()
  ...
  (let ((info (expect-find-info expect-process)))
    ...
    (expect-check-always-event expect-process)
...
(defun expect-push-always (&rest clauses)
  (unless (eq expect-main-cancel 'yes)
    (expect-info-set-always-queries expect-current-info (list clauses))
	...
	(let (moving)
	  ...
	  (save-excursion
	    ...
	    (unless expect-is-running
          (expect-check-always-event process)
...
(defun expect-check-always-event (process)
  (let* ((info (expect-find-info process))
         (tmp-point (expect-info-point info))
         (queries (expect-info-always-queries info))
         clause)
    (while (and queries (listp queries) (car queries))
      (setq clause (car queries))
      (when (and (> (point-max) tmp-point)
                 (goto-char tmp-point)
                 (re-search-forward (car clause) nil t))
        (expect-info-set-point info (point))
        (setq tmp-point (point))
        (save-excursion
          (let* ((expect-current-info info)
                 (old-always-queries (expect-info-always-queries info))
                 (old-queries (expect-info-queries info))
                 expect-cancel)
            (expect-info-set-always-queries info nil)
            ...
            (if expect-cancel
              ...
              (progn
                (expect-info-set-queries info (nconc (expect-info-queries info)
                                                     old-queries))
                (expect-info-set-always-queries info (nconc
                                                      (expect-info-always-queries info)
                                                      old-always-queries))))))
        )
